...
/*
 * Causes the input channel for a network device to accept line lists of wanted HTTP headers.
 */
uint8_t nhttp_collect_headers(uint8_t* devicespec, uint8_t enable_flag) {
	uint8_t unit = nunit(devicespec);
	...
	OS_dcb.dbyt   = 0x00;
	OS_dcb.daux1  = 0;
	OS_dcb.daux2  = enable_flag; // 1 enable, 0 disable
	siov();

	return OS_dcb.dstats;
}

/*
 * Causes the input and output channels of a network device to accept READs for header responses,
 * and WRITEs to request specific headers to the server before the response is sent.
 */
uint8_t nhttp_headers(uint8_t* devicespec, uint8_t enable_flag) {
	uint8_t unit = nunit(devicespec);
